...
export class BusinessRuleRepository {
    ...
    public async create(rule: IBusinessRule): Promise<void> {
        await this.db.query(
            `INSERT INTO business_rules (id, tenant_id, agency_id, field_name, rule_type, criteria) 
             VALUES (?, ?, ?, ?, ?, ?)`,
            [rule.id, rule.tenant_id, rule.agency_id, rule.field_name, rule.rule_type, JSON.stringify(rule.criteria)]
        );
    }
