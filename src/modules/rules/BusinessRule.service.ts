...
export class BusinessRuleService {
    ...
    public async createRule(currentUser: IUser, data: CreateRuleDTO): Promise<IBusinessRule> {
        if (currentUser.user_role === 'VP' && currentUser.agency_id !== data.agency_id) {
            throw new AppError('You can only create rules for your own agency', 403);
        }
    ...
    public async getRulesByAgency(currentUser: IUser, agencyId: string): Promise<IBusinessRule[]> {
        if (currentUser.user_role === 'VP' && currentUser.agency_id !== agencyId) {
